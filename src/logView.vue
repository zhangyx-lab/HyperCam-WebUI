<template>
	<div style="width: 60em; min-width: 40vw; max-width: 100vw; height: 60vh; overflow: hidden; position:relative">
		<div ref="el" log-viewer style="width: 100%; height: 100%; overflow-y: scroll;" @scroll="handleScrollEvent">
			<template v-for="(log, i) in logList" :key="i">
				<div log v-if="filter(log)" :class="[log.src, log.level]">
					<div timestamp style="min-width: 12em">{{ log.timestamp }}</div>
					<div tag>{{ log.src }}:{{ log.level.slice(0, 5).padEnd(5, '&nbsp;') }}</div>
					<div message :class="[log.src, log.level]">{{ log.message }}</div>
				</div>
			</template>
		</div>
		<btn
			...
			style="position: absolute; bottom: 1em; right: 1em;"><i class="fa fa-arrow-down"></i></btn>
	</div>
	<div btn-group>
		<btn type="outlined red" @click="restart(true)">
			<i class="fa fa-sync"></i>
			Server
		</btn>
		<btn type="outlined brand" @click="restart(false)">
			<i class="fa fa-sync"></i>
			Driver
		</btn>
		<btn type="outlined blue" @click="logList.splice(0, logList.length); this.$forceUpdate()">
		...
		<select v-model="filterLevel"
			...
			<option v-for="[key, val] in options" :key="val" :value="val">
...
export default defineComponent({
	setup() {
		return {
			wsAlive, logList, el, autoScroll, filterLevel,
